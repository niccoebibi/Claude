...
- **Gioco degli sposi (settembre 2026):** quiz nel Profilo, con la scheda in fondo alla Home dopo la lista nozze (prima le informazioni importanti). 17 domande su Niccolò e Beatrice, coriandoli a ogni risposta giusta, sette effetti speciali in stile pixel anni '90 assegnati ad alcune domande in `config/quiz.json`, e frasi di consolazione se si sbaglia. **Le risposte giuste non si mostrano mai** (né dopo l'errore né nel riepilogo, e il server non le invia), così gli invitati non se le passano. Trofeo per tutti (grande, accanto alla foto profilo in alto), trofeo brillante per chi indovina tutto; **i primi 3 che indovinano tutto vincono un premio** (scritto in grande; i vincitori li vedono nella Regia). Le domande contengono dati personali: stanno in `config/quiz.json`, escluso da git. Se il file manca, recuperalo dall'anteprima (file `demo/initial.js` dell'Artifact, chiave `settings.quiz`). Va aggiunto al repository solo quando gli sposi l'hanno reso privato.
