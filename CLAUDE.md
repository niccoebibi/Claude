...
- **Gioco degli sposi (settembre 2026):** quiz nel Profilo con pulsante in Home, 19 domande su Niccolò e Beatrice, coriandoli a ogni risposta giusta, tre effetti speciali (drago sputafuoco, pioggia di anelli d'oro, ballerino 8-bit anni '80) assegnati a tre domande in `config/quiz.json`, e frasi di consolazione se si sbaglia. Trofeo per tutti (grande, accanto alla foto profilo in alto), trofeo brillante per chi indovina tutto; **i primi 3 che indovinano tutto vincono un premio** (scritto in grande; i vincitori li vedono nella Regia). Le domande contengono dati personali: stanno in `config/quiz.json`, escluso da git. Se il file manca, recuperalo dall'anteprima (file `demo/initial.js` dell'Artifact, chiave `settings.quiz`). Va aggiunto al repository solo quando gli sposi l'hanno reso privato.
