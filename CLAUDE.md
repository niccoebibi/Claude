...
- **Gioco degli sposi (settembre 2026):** quiz nel Profilo, 15 domande su Niccolò e Beatrice, coriandoli a ogni risposta giusta, trofeo per tutti accanto al nome e trofeo brillante per chi indovina tutto. Le domande contengono dati personali: stanno in `config/quiz.json`, escluso da git. Se il file manca, recuperalo dall'anteprima (file `demo/initial.js` dell'Artifact, chiave `settings.quiz`). Va aggiunto al repository solo quando gli sposi l'hanno reso privato.
